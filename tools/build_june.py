...
SRC="/mnt/files/bp/june/threads"; MONTH="มิ.ย."; Y,MO=2026,6
def dt(s): return datetime.strptime(s,"%Y-%m-%dT%H:%M:%S%z").astimezone(TH)
META=re.compile(r'^ตั้งระยะข้อมูลลูกค้าเป็น')
RECS=[]
bizcount=collections.Counter()
for line in open("/home/user/w/threads.jsonl"):
    line=line.strip()
...
print("threads with June msgs", len(RECS), flush=True)
...
def load(r):
    ...
    return r,out
chats=(load(r) for r in RECS)
print("canned templates",len(CANNED))
def mtype(m):
...
sumrows=[]; extra={}; MF=open("/home/user/w/june_msgs.jsonl","w")
for r,out in chats:
    ...
    if not name:
        for m in out:
            if m["side"]=='ธุรกิจ':
                for n in m["to"]:
                    if n and n!="BioPalm": name=n; break
            elif m["fn"]: name=m["fn"]
            if name: break
    ...
    ftv=None; fav=None
    if fc is not None:
        idx=[i for i,(m,t) in enumerate(conv) if m is fc][0]
        for m,t in conv[idx+1:]:
    ...
    extra[tid]={"ft":ftv,"fa":fav,"qual":hasqual,"stage":stg}
...
json.dump(sumrows,open("/home/user/w/june_sum.json","w"),ensure_ascii=False)
MF.close()
json.dump(extra,open("/home/user/w/june_extra.json","w"),ensure_ascii=False)
